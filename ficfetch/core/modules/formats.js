...
var EPUB =
	{
		createFile: (metadata, chapters) =>
		{
			...
			createCoverImage(metadata["title"], metadata["author"], function (base64img)
			{
				...
				$.each(story_chapters, (index, chapter) =>
				{
		...
		createOPF: (metadata) =>
		{
			...
			let pad = String(metadata["num_chapters"]).length;
			for (let i = 1; i <= metadata["num_chapters"]; i++)
			{
		...
		createNCX: function (metadata, chapters)
		{
			...
			let ncx = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\" ?><!DOCTYPE ncx PUBLIC \" -//NISO//DTD ncx 2005-1//EN\" \"http://www.daisy.org/z3986/2005/ncx-2005-1.dtd\">\n"
		...
		createTitlePage: (metadata) =>
		{
			...
			if (updated == "N/A") updated = "Not Available";
			else if (updated != "Never") updated = formatTimestamp(metadata["date_updated"]);
			...
			return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
				...
				+ "\t\t\t<p><strong>Word Count:</strong> " + (metadata["num_words"] == "N/A" ? "Not Available" : metadata["num_words"]) + "</p>\n"
				+ "\t\t\t<p><strong>Published:</strong> " + (metadata["date_publish"] == "N/A" ? "Not Available" : formatTimestamp(metadata["date_publish"])) + "</p>\n"
				...
				+ "\t\t</div>\n"
				+ "\t\t<small>Generated by <a href=\"https://www.ficfetch.xyz/\">www.ficfetch.xyz</a></small>"
				+ "\t</body>\n"
				+ "</html>";
		},
		createCoverPage: (title) =>
		{
			return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
				+ "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.1//EN\" \"http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd\">\n"
				+ "<html xmlns=\"http://www.w3.org/1999/xhtml\">\n"
		...
		createChapterPage: (title, content) =>
		{
			return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
				+ "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.1//EN\" \"http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd\">\n"
				+ "<html xmlns=\"http://www.w3.org/1999/xhtml\">\n"
...
var HTML =
	{
		...
		{
			let fileName = metadata["title"].replace(/[^a-z0-9]/gi, '_');
			let html_styles = "body { margin: 0; padding: 0; line-height: 2; background: rgb(245, 245, 245); color: rgb(50, 50, 50); font-size: 20px; font-family: Times New Roman, Georgia, serif; }\n"
			...
			createCoverImage(metadata["title"], metadata["author"], (base64img) =>
			{
				let updated = metadata["date_updated"];
				if (updated == "N/A") updated = "Not Available";
				else if (updated != "Never") updated = formatTimestamp(metadata["date_updated"]);
				...
				let file = new Blob([raw], { type: "text/html;charset=utf-8" });
		...
		createTOC: (chapters) =>
		{
			let toc = '<section id="toc"><h2>Table of Contents</h2><ol>';
			let index = 1;
			...
			let pdfDef =
				{
					content:
						[
							{ text: metadata["title"] + "\nby " + metadata["author"], style: 'header' },
							{ text: "Generated by www.ficfetch.xyz", style: 'small', pageBreak: 'after' }
						],
					styles:
						{
							header:
