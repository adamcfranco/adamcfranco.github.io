...
var siyecouk =
	{
		...
		getMetadata: (story_id, callback) =>
		{
			...
			getWebPage(meta_url + "&chapter=1", "(//table/tbody/tr/td/table[3])", null, (results) =>
			{
				results = $(results[0].innerHTML);
				let storyAuthor = $(results.find('h3 font a')[0]).text();
				let linkAuthor = "http://www.siye.co.uk/" + $(results.find('h3 font a')[0]).attr("href");
				results.find('h3 font').remove();
				let storyTitle = results.find('h3').text();
				let storyDesc = String(getMatches(results.text(), /Summary:((.|\n)+)Hitcount:/g, 0));
				let storyGenres = String(getMatches(results.text(), /Genres:([\w \/\,]+)/g, 0));
				let storyRating = String(getMatches(results.text(), /Rating:([\w \-\/\,]+)/g, 0));
				let chapCount = parseInt(results.find("select")[0].length);
				let wordCount = "N/A"
				let publish = "N/A";
				let updated = "N/A";
				let status = "N/A";
				let linkStory = meta_url;
				let storySource = "Sink Into Your Eyes";
				postMessage(1, "Grabbed metadata source successfully.");
				postMessage(2, "Parsing metadata...");
				let metadata = { id: story_id, uuid: guid(), title: storyTitle, author: storyAuthor, description: storyDesc, genre: storyGenres, rating: storyRating, num_words: wordCount, num_chapters: chapCount, date_publish: publish, date_updated: updated, status: status, link_author: linkAuthor, link_story: linkStory, source: storySource };
				callback(metadata);
			});
		},
		getChapters: (story_id, num_chapters, callback) =>
		{
			...
			function GetChapter()
			{
				...
				let chapter_url = "http://www.siye.co.uk/viewstory.php?sid=" + story_id + "&chapter=" + chapter_number;
				getWebPage(chapter_url, "(//table/tbody)[1]", null, (results) =>
				{
					let html = $(results).html();
					let title = $(results).find("select[name='chapter']")[0][chapter_number - 1].innerText.substr(String(chapter_number).length + 1) || "Chapter " + chapter_number;
					let rawContent = $(results).find("td[colspan='2']")[1].innerHTML;
...
var adultfanfictionorg =
	{
		...
		getChapters: function (story_id, num_chapters, callback)
		{
			...
			function GetChapter()
			{
				...
				getWebPage(chapter_url, "//div[@id='contentdata']/table/tbody", null, (results) =>
				{
					let html = $(results).html();
